...
// const Size = ({ handleBack, handleNext }) => {
...
//               <button className="editor-btn-secondary" onClick={() => handleBack()}>Back</button>
//               <button className="editor-btn-primary" onClick={() => handleNext()}>Continue</button>
//             </div>
//           </div>
//         </div>
//       </div>
...
const Size = ({ handleBack, handleNext }) => {
  ...
  const pinchStartZoomRef = useRef(1);
  const pinchStartDistanceRef = useRef(null);
  // NEW: Ref for the crop area to attach manual wheel listener
  ...
  // NEW: Manual wheel listener with { passive: false } to allow preventDefault
  useEffect(() => {
  ...
  }, []);  // Empty deps: runs once on mount. Add [zoom] if you need to recreate, but avoid for perf.
  ...
  // REMOVED: handleWheel (now defined inside useEffect for the listener)
  const handleSliderChange = (e) => {
    setZoom(clampZoom(parseFloat(e.target.value)));
  };
  const handleReset = () => {
      ...
      <div className="editor-container">
        <div className="editor-layout">
          ...
          <div className="editor-left">
            <h2 className="editor-title-left">Crop &amp; Position</h2>
            <div
              ref={cropAreaRef}  // NEW: Attach ref here
              className="editor-crop-area"
              style={{ touchAction: 'none', overflow: 'hidden' }}
            // REMOVED: onWheel={handleWheel}
            >
              <div
                className="editor-image-wrapper"
                onPointerDown={handlePointerDown}
